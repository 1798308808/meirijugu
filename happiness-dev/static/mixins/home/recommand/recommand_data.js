...
export const rd_data ={
	...
	methods: {
		...
		fetchGoodsList(cid,min_id,loadmore) {
			this.$set(this.request_task_group[1], 'data', {type:this.goods_list_type,cid,min_id})
		...
		fetchGoodsListSuccess(loadmore,page,res) {
			console.log('获取分类商品列表成功',res.data.min_id)
		...
		renderCategory(list) {
			...
			this.cover =false
			// 防止加载顺序引发报错
			com.global_watcher_regist('pageChanged',this.swiperPageChanged)
		...
		renderGoodsList(page,list,loadmore){
			...
			if(loadmore){
				...
				this.$set(this.page_group[page],'refresh',false)
			}
			
			this.$set(this.page_group[page],'backup',JSON.parse(JSON.stringify(this.page_group[page].data)))
