...
<script>
	...
	export default {
		...
		onLoad() {
			...
			let token = uni.getStorageSync('needCheckPrivacy')
			if(!token){
				uni.setStorageSync('needCheckPrivacy',true);
				plus.nativeUI.confirm("使用本app前请先查看用户协议和隐私条款", function(e){
					if(e.index==0){
						plus.runtime.openURL('http://www.jufu-unique.com/h5/index.html#/');
					}
				});
			}
		},
